...
def load_artifacts():
    ...
    if not scaler_path.exists() or not features_path.exists():
        raise FileNotFoundError("Preprocessing artifacts missing. Run data preparation first.")
    scaler = joblib.load(scaler_path)
    with open(features_path, 'r') as f:
        feature_columns = json.load(f)
    target_mode = target_mode_path.read_text().strip() if target_mode_path.exists() else 'wind_speed'
...
def load_best_model():
    ...
    for name in ["linear_regression", "ridge_regression", "random_forest"]:
        ...
        if model_path.exists():
            ...
            return model
    raise FileNotFoundError("No trained model found in models/. Train the models first.")


def predict_local_wind_speed(model, forecast_speed, temperature, humidity, pressure,
    ...
    features_scaled = pd.DataFrame(scaler.transform(features_df), columns=feature_columns)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict local wind speed from forecast and weather conditions.")
    parser.add_argument("--forecast_speed", type=float, help="Forecasted wind speed (m/s)")
    ...
    parser.add_argument("--scenario_name", type=str, default="CLI Scenario", help="Name of the scenario")
    parser.add_argument("--verbose", action="store_true", help="Print detailed feature info")
...
def main():
    ...
    if len(sys.argv) > 1:
        args = parse_args()
        pred = predict_local_wind_speed(model, args.forecast_speed, args.temperature, args.humidity, args.pressure,
                                        args.hour, args.day_of_week, args.month)
        print_prediction(args.scenario_name, args.forecast_speed, args.temperature, args.humidity, pred)
    else:
        scenarios = [
            {
        ...
        for scenario in scenarios:
            ...
            print_prediction(name, scenario['forecast_speed'], scenario['temperature'], scenario['humidity'], prediction)
    ...
    print("   python predict_example.py --forecast_speed 10 --temperature 20 --humidity 60 --pressure 1015 --hour 14 --day_of_week 2 --month 7")
